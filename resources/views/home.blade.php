...
<?php
// echo HomeController::check_meal_bookig_status();
?>
<style>
    #table {
        display: block;
...
@section('content')
    ...
    <body style="background-color:#ecf0f5;">
        ...
        <div class="wrapper" class="table-wrapper-scroll-y my-custom-scrollbar">
            ...
            <form method="POST" class="form-inline" action="{{ url('empolyee_details') }}">
                <input name="_token" type="hidden" value="{{ csrf_token() }}" />

                <div class="col-md-3" id="pasding">
                    <div class="form-group">
                        <label><span style="color:red">*</span><?php echo __('Select Employee'); ?> : </label>
                        <input type="text" name="emplyee_seach" class="form-control" id="GrnVendorCode"
                            value="{{ $users->emp_code }}">
                        <input type="hidden" name="emplyee_id" class="form-control" id="emplyee_id"
                            value="{{ $users->id }}">
                        ...
                        <div class="box-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <thead>
                                    <tr>
                                        <th>
                                            <center><?php echo __('Meal Name'); ?></center>
                                        </th>
                                        <th>
                                            <center><?php echo __('Status'); ?></center>
                                        </th>
                                        <th>
                                            <center><?php echo __('Message'); ?></center>
                                        </th>
                                ...
                                <tbody>
                                    @foreach ($meal_name as $meal_names)
                                        ...
                                        ?>
                                        <tr>
                                            <td><b>{{ $meal_names->menu_name }}<small> {{ $print_date }}</small> </b>
                                            </td>
                                            <td>
                                                <center><?php if ($available == true) {
                                                    if ($status == false) { ?><a
                                                        class="btn btn-primary btn-sm"
                                                        href="{{ url('meal_booking/' . $meal_names->id . '/' . $users->id) }}">Book</a><?php } else { ?> <a
                                                        class="btn btn-danger btn-sm"
                                                        href="{{ url('meal_booking_delete/' . $meal_names->id . '/' . $users->id) }}">Cancel</a>
                                                    <?php }
                                                    } ?></center>
                                            </td>
                                            <td>
                                                <center><?php if ($checkmsg_time == true) {
                                                    if ($status == true) { ?>Booking Confirmed <?php }
                                                    } ?></center>
                                            </td>
                        ...
                        <div class="box-body">
                            <table class="table table-bordered" style="width:100%" id="table">
                                ...
                                @foreach ($meal_name as $meal_namess)
                                    ...
                                    <tr>
                                        <td>
                                            <center><b>{{ $meal_namess['menu_name'] }}</b></center>
                                        </td>
                                        <td>
                                            <center>{{ $current_month['count'] }}</center>
                                        </td>
                                        <td>
                                            <center>{{ $current_month['amount'] }}</center>
                                        </td>
                                        <td>
                                            <center>{{ $previous_month['count'] }}</center>
                                        </td>
                                        <td>
                                            <center>{{ $previous_month['amount'] }}</center>
                                        </td>

                                    </tr>
                                    <?php if (is_numeric($current_month['amount']) &&
                                    is_numeric($previous_month['amount'])) {
                                ...
                                <tr>
                                    ...
                                    <th colspan="2">
                                        <center>{{ $current_month_total_amount }}</center>
                                    </th>
                                    <th colspan="2">
                                        <center>{{ $previous_month_total_amount }}</center>
                                    </th>
